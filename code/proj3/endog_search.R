...
mod_list <- mapply(function(x, y) ergm(net ~ edges +  
                             gwdegree(x,fixed=T) + gwesp(y,fixed=T),
                           estimate=c( "CD"),control = control.ergm(CD.nsteps = 16,CD.multiplicity = 2,MPLE.samplesize=50000,
                                                                    parallel=16,parallel.type = 'PSOCK',
                                           MCMC.max.maxedges = as.numeric(summary(net~edges))[1] *2)),
                   
                   runif(100,0.01,3),runif(100,0.01,3))


gof_list = lapply(mod_list,function(x) btergm::gof(x,nsim=160,parallel = 'multicore',ncpus = 16))

save.image('endog_search.RData')
