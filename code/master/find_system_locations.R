...
#system_df[grep('NORTHEAST HARRIS COUNTY MUD 1 ',system_df$PWS_NAME),c('x','y')] <- 
...
system_df[grep("HARRIS MONTGOMERY COUNTIES MUD 386",system_df$PWS_NAME),c('x','y')] <- t(sapply(1:sum(grepl("HARRIS MONTGOMERY COUNTIES MUD 386",system_df$PWS_NAME)),function(x) coordinates(twd_centroid[twd$NAME=="HARRIS MONTGOMERY COUNTIES MUD 386"])))
system_df[grep("WOODLANDS MUD 1",system_df$PWS_NAME),c('x','y')] <- t(sapply(1:sum(grepl("MONTGOMERY COUNTY MUD 40|THE WOODLANDS MUD 2",system_df$PWS_NAME)),function(x) coordinates(twd_centroid[twd$NAME=="MONTGOMERY COUNTY MUD 40|THE WOODLANDS MUD 2"])))
system_df[grep("CITY OF PEARLAND MUD 1",system_df$PWS_NAME),c('x','y')] <- t(sapply(1:sum(grepl("CITY OF PEARLAND MUD 1",system_df$PWS_NAME)),function(x) coordinates(twd_centroid[twd$NAME=="CITY OF PEARLAND MUD 1"])))
system_df[grep("ROSHARON TOWNSHIP",system_df$PWS_NAME),c('x','y')] <- t(sapply(1:sum(grepl("ROSHARON TOWNSHIP",system_df$PWS_NAME)),function(x) coordinates(twd_centroid[twd$NAME=="ROSHARON TOWNSHIP"])))
...
rbind(coordinates(gCentroid(twd[twd@data$NAME=='SUNBELT FWSD',])))
test <- coordinates(gCentroid(twd[twd@data$NAME=='SUNBELT FWSD',]))

test
tt = system_df[is.na(system_df$x)&grepl('SUNBELT FWSD',system_df$PWS_NAME),c('x','y')]
tt[1:nrow(tt),]<-test
tt[=2]
tsummary(system_df[,c('x','y')])
system_df[is.na(system_df$x)&grepl('SUNBELT FWSD',system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(twd[twd@data$NAME=='SUNBELT FWSD',]))
system_df[is.na(system_df$x)&grepl('RIMWICK FOREST',system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(montgomery_sub[grep("RIMWICK FOREST",montgomery_sub@data$NAME),]))
system_df[is.na(system_df$x)&grepl("ALLENWOOD SUBDIVISION",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(montgomery_sub[grep("ALLENWOOD 0",montgomery_sub@data$NAME),]))
system_df[is.na(system_df$x)&grepl("HUNTERS RETREAT",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(montgomery_sub[grep("HUNTERS RETREAT",montgomery_sub@data$NAME),]))
system_df[is.na(system_df$x)&grepl("KIPLING OAKS [0-9]",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(montgomery_sub[grep("KIPLING OAKS [0-9]",montgomery_sub@data$NAME),]))
system_df[is.na(system_df$x)&grepl("TIMBERGREEN",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(montgomery_sub[grep("TIMBERGREEN",montgomery_sub@data$NAME),]))
system_df[is.na(system_df$x)&grepl("ARMADILLO",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(montgomery_sub[grep("ARMADILLO",montgomery_sub@data$NAME),]))
system_df[is.na(system_df$x)&grepl("TOWERING OAKS",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(montgomery_sub[grep("TOWERING OAKS",montgomery_sub@data$NAME),]))
system_df[is.na(system_df$x)&grepl("DEER RIDGE",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(montgomery_sub[grep("DEER RIDGE",montgomery_sub@data$NAME),]))
system_df[is.na(system_df$x)&grepl("AUSTIN COUNTY WSC",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(tpuc[grep("AUSTIN COUNTY WSC",tpuc@data$NAME),]))
system_df[is.na(system_df$x)&grepl("TBCD",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(twd[grep("TRINITY BAY CONSERVATION DISTRICT",twd@data$NAME),]))
system_df[is.na(system_df$x)&grepl("G & W WSC",system_df$PWS_NAME,fixed=T),c('x','y')] <- coordinates(gCentroid(tpuc[grep("G & W WSC",tpuc@data$NAME,fixed=T),]))
system_df[is.na(system_df$x)&grepl("HUNTERS COVE SEC 1",system_df$PWS_NAME,fixed=F),c('x','y')] <- coordinates(gCentroid(tpuc[grep("HUNTERS COV",tpuc@data$NAME,fixed=F),]))
system_df[is.na(system_df$x)&grepl("VILLAGE OF SURFSIDE BEACH",system_df$PWS_NAME,fixed=F),c('x','y')] <- coordinates(gCentroid(tpuc[grep("VILLAGE OF SURFSIDE",tpuc@data$NAME,fixed=F),]))
system_df[is.na(system_df$x)&grepl("CITY OF FREEPORT ",system_df$PWS_NAME,fixed=F),c('x','y')] <- coordinates(gCentroid(places[grep("FREEPORT",places@data$NAME,fixed=F),]))
system_df[is.na(system_df$x)&grepl("CITY OF SUGAR LAND  ",system_df$PWS_NAME,fixed=F),c('x','y')] <- coordinates(gCentroid(places[grep("SUGAR LAND",places@data$NAME,fixed=F),]))
system_df[is.na(system_df$x)&grepl("THUNDERBIRD",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(twd[grep("THUNDERBIRD",twd@data$NAME),]))
system_df[is.na(system_df$x)&grepl("ORANGE GROVE",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(tpuc[grep("ORANGE GROVE",tpuc@data$NAME),]))
system_df[is.na(system_df$x)&grepl("PALOMA ACRES",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(brazoria_sub[grep("PALOMA ACRES",brazoria_sub@data$NAME),]))
system_df[is.na(system_df$x)&grepl("ROBIN COVE",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(brazoria_sub[grep("ROBIN COVE",brazoria_sub@data$NAME),]))
system_df[is.na(system_df$x)&grepl("COUNTRY OAKS",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(brazoria_sub[grep("COUNTRY OAKS",brazoria_sub@data$NAME),]))
system_df[is.na(system_df$x)&grepl("^WOOD OAKS",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(brazoria_sub[grep("^WOOD OAKS",brazoria_sub@data$NAME),]))
system_df[is.na(system_df$x)&grepl("^LEE RIDGE",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(brazoria_sub[grep("^LEE RIDGE",brazoria_sub@data$NAME),]))
system_df[is.na(system_df$x)&grepl("^SNUG HARBOR",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(brazoria_sub[grep("^SNUG HARBOR",brazoria_sub@data$NAME),]))
system_df[is.na(system_df$x)&grepl("^MEADOWVIEW",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(brazoria_sub[grep("^MEADOWVIEW",brazoria_sub@data$NAME),]))
system_df[is.na(system_df$x)&grepl("^ROYAL RIDGE",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(brazoria_sub[grep("^ROYAL RIDGE",brazoria_sub@data$NAME),]))
system_df[is.na(system_df$x)&grepl("SUN RANCH",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(tpuc[grep("SUN RANCH",tpuc@data$NAME),]))
system_df[is.na(system_df$x)&grepl("CITY OF MISSOURI CITY MUSTANG",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(places[grep("MISSOURI",places@data$NAME),]))
system_df[is.na(system_df$x)&grepl("CITY OF SUGAR LAND",system_df$PWS_NAME),c('x','y')] <- coordinates(gCentroid(places[grep("SUGAR LAND",places@data$NAME),]))

library(ggmap)
library(stringr)
geo_other= cbind(PWS_NAME = system_df$PWS_NAME[is.na(system_df$x)],
                   geocode(paste(gsub(' WATER SYSTEM',' ',system_df$PWS_NAME[is.na(system_df$x)]),', ',str_to_title(system_df$County[is.na(system_df$x)]),' County, TX',sep='')))
...
system_df[system_df$x>10000,] %>% select(PWS_NAME,x,y)
