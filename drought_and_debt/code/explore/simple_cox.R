...
twd_boundaries <- st_read('bosquebox/spatial_inputs/Service_Area_Boundaries/PWS_shapefile/PWS_Export.shp')
...
drought_dt <- fread('input/MR_drought_scores.csv')
...
demos <- readRDS('input/pws_demos_MR.RDS')
tx_cox <- left_join(tx_cox,demos)

# DWV source: interconnection counts + total storage capacity (TSTC, MG) come
# from the measures/purchases widgets (storage_connections_data.txt), not the
# population/flow-rate tables.
sc <- fread('input/storage_connections_data.txt')
sc$PWS_ID <- as.character(sc$PWS_ID)
inters <- sc[Var=='TSTC',.(Interconnections = as.numeric(Num_Interconnections)[1]),by=.(PWS_ID)]
tx_cox <- left_join(tx_cox,inters)

# Population served: DWV master total + residential share. pws_population.RDS is
# now one row per system (Population_Served, Pop_Residential, Pop_Wholesale).
population <- as.data.table(readRDS('input/pws_population.RDS'))
for (col in c('Pop_Residential','Pop_Wholesale'))
  if (!col %in% names(population)) population[[col]] <- 0
population[is.na(Pop_Residential), Pop_Residential := 0]
population[is.na(Pop_Wholesale),   Pop_Wholesale   := 0]
pop_df <- population[,.(Population_Served = as.numeric(Population_Served),
                        Perc_Residential = 100 * Pop_Residential /
                          pmax(Pop_Residential + Pop_Wholesale, 1)), by=.(PWS_ID)]
tx_cox <- left_join(tx_cox,pop_df)

# Storage capacity (TSTC, MG) from the interconnects file; average daily usage
# (DWV ADU flow-rate) from pws_storage.RDS, normalized to gallons/day using the
# per-system unit that 05_scrape_storage_and_pops.R now carries (DWV can report
# MGD or GPM, not just MGD, so we don't assume a fixed unit).
storage_cap <- sc[Var=='TSTC',.(Storage_MG = as.numeric(Value)[1]),by=.(PWS_ID)]
storage <- as.data.table(readRDS('input/pws_storage.RDS'))
storage$PWS_ID <- as.character(storage$PWS_ID)
adu <- as.numeric(storage$Avg_Daily_Usage)
u   <- toupper(trimws(storage$Avg_Daily_Usage_Unit))
mult <- fifelse(u == 'MGD', 1e6,          # million gallons/day
         fifelse(u == 'GPD', 1,           # gallons/day
          fifelse(u == 'GPM', 60 * 24,    # gallons/minute -> /day
           NA_real_)))
if (any(is.na(mult) & !is.na(adu)))
  warning('Unhandled ADU flow-rate unit(s): ',
          paste(unique(u[is.na(mult) & !is.na(adu)]), collapse = ', '))
storage$Avg_Daily_Consumption <- adu * mult
tx_cox <- left_join(tx_cox,storage_cap)
tx_cox <- left_join(tx_cox,storage[,.(PWS_ID,Avg_Daily_Consumption)])

pwsd <- fread('bosquebox/input/SDWA_latest_downloads/SDWA_PUB_WATER_SYSTEMS.csv')
...
owners <- readRDS('bosquebox/input/texas_dww/pws_details_2023-03-21.RDS')
...
stargazer(cox_MR_duplicate,notes = '*Only systems with >3,300 connections + a drought filing with TCEQ',
            type = 'html',out = 'output/mullin_rubado_almost_repliation.html',style = 'ajps',
