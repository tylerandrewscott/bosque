...
ggsave(figure1,filename = 'output/figure1.png',dpi = 500,width=6,height=2.8,units='in')
#Drought score
...
twd_boundaries = st_read('bosquebox/spatial_inputs/Service_Area_Boundaries/PWS_Export.shp')
...
library(tidycensus)
tx_systems = fread("bosquebox/input/epa_sdwis/tx_water_system_geographic_area_2-9-19.csv",stringsAsFactors = F)
...
tx_info = as.data.table(readRDS('bosquebox/input/texas_dww/pws_details_2019-07-02.RDS'))
...
interconnects = fread('bosquebox/input/texas_dww/sales_connections_2018-07-03.csv')
...
dinfo = fread('bosquebox/input/twdd_records/district_list_2019-03-08.csv',stringsAsFactors = F)
...
library(readxl)
tx_restrictions = read_excel('bosquebox/input/tceq_drought_notices.xlsx') %>% rename(PWS_ID = ID)# %>% filter(!is.na(PWS_ID))
...
urban_area = readRDS('bosquebox/input/urban_area_proportions.RDS')
...
library(tigris)
# terra/sf replacement for the retired spTransform()/CRS() idiom.
# Builds the TX county layer, reprojects (terra), writes the INLA adjacency file.
tx_county_sp = tx_county_adjacency(crs = albersNA, queen = FALSE)
...
acs_pull_dt = readRDS('bosquebox/input/raw_acs_tract_data.RDS')
...
cox_base = inla(inla.surv(time = Time,event = Event) ~ -1 + intercept + 
                      log(Connections+0.001) + log(Total_Storage_MG+0.001) + 
                      ...
                      data = cox_mud,family = 'coxph',control.compute = list(waic=T,dic=T),
                      control.hazard=list(model="rw1", n.intervals=months,scale.model = TRUE))
...
cox_dt = readRDS('scratch/full_panel_data.RDS')
start_date = mdy('5/4/2010')
start_date_dec = decimal_date(start_date)
end_date = mdy('7/7/2015')
cox_dt <- cox_dt[District_Type=='MUD',]
cox_base = readRDS('scratch/base_inlacoxph_object.RDS')
...
saveRDS(cox_dt,'scratch/data_used_in_model.RDS')
...
saveRDS(gof_table,'scratch/gof_table.RDS')
sapply(saveobjs,function(x) saveRDS(fin_mods[[x]],paste0('scratch/',mods[x],'.RDS')))
...
#saveRDS(fin_mods,'scratch/mod_object_list.RDS')
