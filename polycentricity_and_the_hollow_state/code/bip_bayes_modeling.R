...
net %v% 'GCD_Type' <- ifelse(!is.na(net %v% 'GCD'),paste(net %v% 'GCD', net %v% 'Water_Type'),NA)
...
net %v% 'GCD_GW' <- ifelse(is.na(net %v% 'GCD_Type'),NA,ifelse(!grepl("NONE",net %v% 'GCD_Type') &grepl('GW$',net %v% 'GCD_Type'),net %v% 'GCD_Type','NONE'))
...
load('scratch_bip_bayes.RData')

summary(bmod)

Bergm::bergm.output(bmod)
# 
# 
